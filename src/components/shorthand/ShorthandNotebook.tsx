  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-slate-100">Personal Shorthand Notebook</h3>
          <p className="text-xs text-gray-500 dark:text-slate-400">Save difficult outlines, custom phrases, rules & personal mistakes.</p>
        </div>
      ...
      {isAdding && (
        <form onSubmit={handleSave} className="p-5 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-purple-500/40 space-y-4">
          <h4 className="text-xs font-bold uppercase tracking-wider text-purple-300">New Note</h4>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              <label className="text-[11px] text-gray-500 dark:text-slate-400 font-medium block mb-1">Title / Word / Phrase</label>
              <input
                type="text"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder="e.g. In accordance with the law / Difficult joining"
                className="w-full px-3 py-2 rounded-lg bg-white dark:bg-slate-950 border border-gray-200 dark:border-slate-800 text-xs text-gray-900 dark:text-slate-100 focus:outline-none focus:border-purple-500"
                required
              />
            </div>
            <div>
              <label className="text-[11px] text-gray-500 dark:text-slate-400 font-medium block mb-1">Category</label>
              <select
                value={category}
          ...
          <div>
            <label className="text-[11px] text-gray-500 dark:text-slate-400 font-medium block mb-1">Outline details / Reminders / Transcription</label>
            <textarea
              value={content}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <input
          ...
          className="px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 text-xs text-gray-900 dark:text-slate-100 focus:outline-none focus:border-purple-500"
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {filteredNotes.map((n) => (
          <div key={n.id} className="p-4 rounded-xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 space-y-2 relative group">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h4 className="font-bold text-gray-900 dark:text-slate-100 text-sm mt-1.5">{n.title}</h4>
              </div>
              <button
                onClick={() => handleDelete(n.id)}
                className="text-gray-400 dark:text-slate-500 hover:text-red-400 text-xs font-bold transition-colors p-1"
                title="Delete note"
              >
                ✕
              </button>
            </div>
            <p className="text-xs text-gray-600 dark:text-slate-300 leading-relaxed font-sans">{n.content}</p>
      ...
      {filteredNotes.length === 0 && (
        <div className="text-center py-10 rounded-xl bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800/80 text-xs text-gray-500 dark:text-slate-400">

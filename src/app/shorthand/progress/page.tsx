    ...
    <div className="min-h-screen bg-white dark:bg-[#070B14] text-gray-900 dark:text-slate-100">
      ...
      <main className="container-main py-10 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 text-center">
            <span className="text-[10px] uppercase font-bold text-amber-400">Current Streak</span>
            <div className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-slate-100 mt-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-6 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 space-y-2">
            <span className="text-xs font-bold uppercase tracking-wider text-purple-400">Audio Experience</span>
            <div className="text-2xl font-black text-gray-900 dark:text-slate-100">{progress.totalWordsDictated} Words</div>
            <p className="text-xs text-gray-500 dark:text-slate-400">Total volume of spoken English dictation words practiced.</p>
          </div>

          <div className="p-6 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 space-y-2">
            <span className="text-xs font-bold uppercase tracking-wider text-sky-400">Keyboard Transcription</span>
            <div className="text-2xl font-black text-gray-900 dark:text-slate-100">{progress.totalWordsTranscribed} Words</div>
            <p className="text-xs text-gray-500 dark:text-slate-400">Total words typed and evaluated on computer keyboard.</p>
          </div>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 space-y-4">
          <div className="flex items-center justify-between border-b border-gray-200 dark:border-slate-800 pb-3">
            <h3 className="text-base font-bold text-gray-900 dark:text-slate-100">Recent Stenography Exam Attempts</h3>
          ...
          {progress.testHistory.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left text-gray-600 dark:text-slate-300">
                <thead className="text-[10px] uppercase tracking-wider text-gray-500 dark:text-slate-400 bg-white dark:bg-slate-950 border-b border-gray-200 dark:border-slate-800">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800">
                  {progress.testHistory.map((t) => (
                    <tr key={t.id} className="hover:bg-gray-100 dark:hover:bg-slate-800/40">
                      <td className="p-3 text-gray-500 dark:text-slate-400">{new Date(t.date).toLocaleDateString()}</td>
                      <td className="p-3 font-semibold text-gray-700 dark:text-slate-200">{t.examName}</td>
                      ...
                      <td className="p-3 font-bold text-gray-900 dark:text-slate-100">{t.accuracyPercentage}%</td>

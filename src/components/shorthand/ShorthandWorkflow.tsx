    ...
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {steps.map((s, idx) => (
          <div
            key={s.num}
            className="p-5 rounded-2xl bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 relative space-y-3"
          >
            <div className="flex items-center justify-between">
              <span className="text-2xl">{s.icon}</span>
              <span className="text-xs font-black text-gray-500 dark:text-slate-400 tracking-wider">STEP {s.num}</span>
            </div>
            <div>
              <h4 className="font-bold text-gray-900 dark:text-slate-100 text-sm">{s.title}</h4>
              <p className="text-xs text-gray-500 dark:text-slate-400 mt-1 leading-relaxed">{s.subtitle}</p>
            </div>
            {idx < steps.length - 1 && (
      ...
      <div className="p-8 rounded-3xl bg-gradient-to-r from-purple-950/40 via-slate-900 to-sky-950/30 border border-purple-500/30 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          ...
          <Link
            href="/test"
            className="px-5 py-2.5 rounded-xl bg-gray-100 dark:bg-slate-800 hover:bg-slate-700 text-gray-700 dark:text-slate-200 font-semibold text-xs border border-gray-300 dark:border-slate-700 transition-colors"
          >
